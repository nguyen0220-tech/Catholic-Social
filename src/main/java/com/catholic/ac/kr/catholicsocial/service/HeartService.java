...
public class HeartService {
    ...
    public ListResponse<HeartDTO> getHeartsByMomentId(Long currentUserId, Long momentId, int page, int size) {
        ...
        List<HeartProjection> rs = hearts.stream()
                .filter(h -> !setBlocked.contains(h.getUserId()))
                .toList();

        return new ListResponse<>(HeartMapper.toListHeartDTO(rs));
    }
